...
const next = nextAfter('/history/the-ghettos');

/**
 * The ghettos, 1939 to 1942: the deep version of the first topic in the
 * Holocaust chapter of the Second World War overview. The community that was
 * there before, the first German orders, Lodz and Warsaw one after the other,
 * how a ghetto actually ran from the inside, the hundreds of small ones nobody
 * pictures, the emptying of 1942, the record the prisoners buried, and where a
 * visitor can stand now.
 *
 * The General Government, the camps and the Warsaw ghetto uprising of 1943 have
...
const frames = computed<Frame[]>(() => group<Frame[]>('ghettos.frames') ?? []);

/**
 * One frame carries an upright object rather than a scene, the ghetto money
 * printed in Litzmannstadt and signed by the man the Germans made responsible
 * for the place, and it is worth reading properly, so it gets its own plate
 * with the same picture behind it dimmed to a texture.
 */
const exhibits = new Set(['lodz']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The Warsaw frame carries the plan of the quarter as the occupier's own Polish
 * language newspaper printed it in October 1940, which opens full screen like
 * any other exhibit, because nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'warsaw';
...
<template>
    <Head :title="t('ghettos.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('ghettos.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/ghettos-hero-sm.avif  960w,
                        /images/ghettos-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/ghettos-hero-sm.jpg"
                    srcset="
                        /images/ghettos-hero-sm.jpg  960w,
                        /images/ghettos-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/ghettos-${frame.photo}-sm.avif 960w, /images/ghettos-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/ghettos-${frame.photo}-sm.jpg`"
                                :srcset="`/images/ghettos-${frame.photo}-sm.jpg 960w, /images/ghettos-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/ghettos-map-sm.jpg"
                                        srcset="/images/ghettos-map-sm.jpg 960w, /images/ghettos-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/ghettos-map-lg.jpg"
                                        with-caption
                                        :alt="t('ghettos.map.alt')"
                                        :caption="t('ghettos.map.caption')"
                                        :credit="t('ghettos.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/ghettos-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/ghettos-${frame.photo}-sm.jpg 960w, /images/ghettos-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/ghettos-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('ghettos.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('ghettos.sources.body') }}
                </p>
