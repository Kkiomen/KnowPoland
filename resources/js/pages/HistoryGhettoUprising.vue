...
const next = nextAfter('/history/warsaw-ghetto-uprising');

/**
 * The Warsaw Ghetto Uprising of 1943: the deep version of the third topic in
 * the Holocaust chapter of the Second World War overview. What was left of the
 * ghetto after the deportations of 1942, the two fighting organisations and
 * the weapons they did and did not get, the four days of January 1943, the
 * bunkers, 19 April, the burning of the district, 18 Mila street and the
 * sewers, the false total in the German report, and what stands there now.
 *
 * The closed districts, the killing centres and the Polish rising of 1944 are
 * other articles: this page only says plainly that 1943 and 1944 are two
 * different risings, because foreign readers confuse them constantly.
 *
...
const frames = computed<Frame[]>(
    () => group<Frame[]>('ghettouprising.frames') ?? [],
);

/**
 * One frame carries a document rather than a scene: the aerial reconnaissance
 * photograph taken over the district after the fighting, which opens full
 * screen, because nobody can read a city from the air at panel size.
 */
const hasAerial = (key: string): boolean => key === 'burning';

/** A frame has a side panel when it holds that photograph. */
const hasPanel = (key: string): boolean => hasAerial(key);
...
<template>
    <Head :title="t('ghettouprising.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('ghettouprising.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/ghettouprising-hero-sm.avif  960w,
                        /images/ghettouprising-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/ghettouprising-hero-sm.jpg"
                    srcset="
                        /images/ghettouprising-hero-sm.jpg  960w,
                        /images/ghettouprising-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/ghettouprising-${frame.photo}-sm.avif 960w, /images/ghettouprising-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/ghettouprising-${frame.photo}-sm.jpg`"
                                :srcset="`/images/ghettouprising-${frame.photo}-sm.jpg 960w, /images/ghettouprising-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        src="/images/ghettouprising-aerial-sm.jpg"
                                        srcset="/images/ghettouprising-aerial-sm.jpg 960w, /images/ghettouprising-aerial-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/ghettouprising-aerial-lg.jpg"
                                        with-caption
                                        :alt="t('ghettouprising.aerial.alt')"
                                        :caption="
                                            t('ghettouprising.aerial.caption')
                                        "
                                        :credit="
                                            t('ghettouprising.aerial.credit')
                                        "
                                    />
                                </div>
                            </div>
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('ghettouprising.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('ghettouprising.sources.body') }}
                </p>
