...
import BordersMap from '@/components/BordersMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/the-borders-moved');

/**
 * How Poland ended up several hundred kilometres west of where it had been:
 * Tehran, Yalta and Potsdam, the cities lost in the east, the German cities
 * taken in the west and renamed, the Poles moved out of the Kresy, the
 * Germans expelled, Operation Vistula, the Jewish survivors who left, the
 * library that followed its readers to Wroclaw, and what a visitor can read
 * off a wall today. Told as a filmstrip: scrolling down drives the frames
...
const frames = computed<Frame[]>(() => group<Frame[]>('borders.frames') ?? []);

/**
 * Most of the evidence here is a printed sheet: a conference photograph, an
 * expulsion order, an exhibition catalogue. Cropped to fill a widescreen
 * frame all of them become unreadable, so each is shown whole, on its own
...
/**
 * The frame on what was lost carries the map of the move, and its backdrop is
 * dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'kresy';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * The map and the landscape pictures get the wide panel; the upright
 * expulsion order of July 1945 fits the narrow one.
 */
const isWide = (key: string): boolean => key !== 'expulsion';
...
<template>
    <Head :title="t('borders.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('borders.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/borders-hero-sm.avif  960w,
                        /images/borders-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/borders-hero-sm.jpg"
                    srcset="
                        /images/borders-hero-sm.jpg  960w,
                        /images/borders-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/borders-${frame.photo}-sm.avif 960w, /images/borders-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/borders-${frame.photo}-sm.jpg`"
                                :srcset="`/images/borders-${frame.photo}-sm.jpg 960w, /images/borders-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/borders-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('borders.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('borders.sources.body') }}
                </p>
