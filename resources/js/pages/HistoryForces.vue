...
const next = nextAfter('/history/polish-forces-abroad');

/**
 * Polish forces abroad: the deep version of the fifth topic in the resistance
 * chapter of the Second World War overview. How an army got out of a country
 * that had been overrun, the Battle of Britain and 303 Squadron, the navy that
 * sailed before the first shot, the army that walked out of the Soviet Union,
 * Monte Cassino, Falaise and Breda under Stanislaw Maczek, the paratroopers at
 * Arnhem, the second Polish army raised in the east under Zygmunt Berling, and
 * what happened to all of them when the war ended and the country they had been
 * fighting for was not the one they could go back to.
 *
 * The London government, the Sikorski-Maisky agreement and Anders taking his
 * army out of the Soviet Union belong to the article on the government in
 * exile: they are bridged here in a line rather than retold. The Warsaw Uprising
 * of 1944 is the article after this one. Two disputes are carried openly rather
 * than picked: the 303 Squadron score, wartime claim and later revision both
 * attributed, and the blame put on Sosabowski after Arnhem, which the page gives
 * along with its withdrawal.
 *
 * Nine frames, then a short note on what is left to visit, then the sources.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('forces.frames') ?? []);

/**
 * One frame carries the real map beside the text: the artillery fire plan the
 * Polish Second Corps printed of the ground it fought over. It opens full
 * screen, because nobody can read a 1 to 25,000 sheet at panel size.
 */
const hasMap = (key: string): boolean => key === 'cassino';
...
<template>
    <Head :title="t('forces.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('forces.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/forces-hero-sm.avif  960w,
                        /images/forces-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/forces-hero-sm.jpg"
                    srcset="
                        /images/forces-hero-sm.jpg  960w,
                        /images/forces-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/forces-${frame.photo}-sm.avif 960w, /images/forces-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/forces-${frame.photo}-sm.jpg`"
                                :srcset="`/images/forces-${frame.photo}-sm.jpg 960w, /images/forces-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                        ...
                        >
                            <div
                                class="grid gap-8 lg:grid-cols-[minmax(0,46ch)_minmax(0,34rem)] lg:items-end"
                            >
                                ...
                                >
                                    <ExhibitImage
                                        src="/images/forces-map-sm.jpg"
                                        srcset="/images/forces-map-sm.jpg 960w, /images/forces-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/forces-map-lg.jpg"
                                        with-caption
                                        :alt="t('forces.map.alt')"
                                        :caption="t('forces.map.caption')"
                                        :credit="t('forces.map.credit')"
                                    />
                                </div>
                            </div>
                        </div>
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('forces.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('forces.sources.body') }}
                </p>
