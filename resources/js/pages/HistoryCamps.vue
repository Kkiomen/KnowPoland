...
const next = nextAfter('/history/the-death-camps');

/**
 * The death camps, 1941 to 1945: the deep version of the second topic in the
 * Holocaust chapter of the Second World War overview. How the decision was
 * taken and where it was carried out, Operation Reinhard, what a transport
 * actually was, Auschwitz-Birkenau, Majdanek and Operation Harvest Festival,
 * how the world was told, the three revolts, the erasure and the evidence
 * that outlived it, and how to stand at these places now.
 *
 * The closed districts of 1939 to 1942, the German colony in central Poland
 * and the rising of April 1943 have their own articles here and are only
 * bridged to.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('camps.frames') ?? []);

/**
 * One frame carries a document rather than a scene, the coded message in which
 * the German commander in Lublin counted his own dead at the end of 1942, and
 * it is worth reading line by line, so it gets its own plate with the same
 * picture behind it dimmed to a texture.
 */
const exhibits = new Set(['evidence']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The opening frame carries the wall map the American prosecution put in
 * evidence at Nuremberg in 1947, which opens full screen like any other
 * exhibit, because nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'decision';
...
<template>
    <Head :title="t('camps.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('camps.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/camps-hero-sm.avif  960w,
                        /images/camps-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/camps-hero-sm.jpg"
                    srcset="
                        /images/camps-hero-sm.jpg  960w,
                        /images/camps-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/camps-${frame.photo}-sm.avif 960w, /images/camps-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/camps-${frame.photo}-sm.jpg`"
                                :srcset="`/images/camps-${frame.photo}-sm.jpg 960w, /images/camps-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/camps-map-sm.jpg"
                                        srcset="/images/camps-map-sm.jpg 960w, /images/camps-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/camps-map-lg.jpg"
                                        with-caption
                                        :alt="t('camps.map.alt')"
                                        :caption="t('camps.map.caption')"
                                        :credit="t('camps.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/camps-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/camps-${frame.photo}-sm.jpg 960w, /images/camps-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/camps-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('camps.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('camps.sources.body') }}
                </p>
