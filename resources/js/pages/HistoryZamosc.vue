...
const next = nextAfter('/history/zamosc-expulsions');

/**
 * The Zamosc expulsions, 1942 to 1943: the deep version of the fourth topic in
 * the occupation chapter of the Second World War overview. The plan for the
 * east and why this region was picked to test it, the first night and the
 * transit camp and the sorting into four groups, the children, the trains sent
 * north and the people who met them, the German colonists, the two undergrounds
 * that fought, the reprisals, the postwar search for the missing children, and
 * what a visitor sees now.
 *
 * The General Government, the government in exile and the camps have their own
 * articles, and they are only bridged to here.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('zamosc.frames') ?? []);

/**
 * One frame carries an upright period photograph rather than a scene, the trek
 * of Bessarabian German families waiting to be moved into somebody else's
 * farms, and it is worth looking at properly, so it gets its own plate with the
 * same picture behind it dimmed to a texture.
 */
const exhibits = new Set(['settlers']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the first night carries the published map of the region, drawn
 * village by village for the 1979 documentary edition, which opens full screen
 * like any other exhibit, because nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'night';
...
<template>
    <Head :title="t('zamosc.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('zamosc.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/zamosc-hero-sm.avif  960w,
                        /images/zamosc-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/zamosc-hero-sm.jpg"
                    srcset="
                        /images/zamosc-hero-sm.jpg  960w,
                        /images/zamosc-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/zamosc-${frame.photo}-sm.avif 960w, /images/zamosc-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/zamosc-${frame.photo}-sm.jpg`"
                                :srcset="`/images/zamosc-${frame.photo}-sm.jpg 960w, /images/zamosc-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/zamosc-map-sm.jpg"
                                        srcset="/images/zamosc-map-sm.jpg 960w, /images/zamosc-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/zamosc-map-lg.jpg"
                                        with-caption
                                        :alt="t('zamosc.map.alt')"
                                        :caption="t('zamosc.map.caption')"
                                        :credit="t('zamosc.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/zamosc-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/zamosc-${frame.photo}-sm.jpg 960w, /images/zamosc-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/zamosc-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('zamosc.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('zamosc.sources.body') }}
                </p>
