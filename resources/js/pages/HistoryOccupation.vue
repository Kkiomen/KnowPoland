...
const next = nextAfter('/history/occupation-and-holocaust');

/**
 * The five and a half years between the end of the September campaign and the
 * end of the war: the two occupation systems, the killing of the Polish
 * elites, the Soviet deportations east, daily life under German rule, the
 * ghettos, the killing centres, Jewish resistance, the Poles who helped and
 * the Poles who did not, the arithmetic of the losses, and how to behave at
 * the memorial sites today. Told as a filmstrip: scrolling down drives the
 * frames sideways. On a phone the strip becomes an ordinary vertical stack,
 * because horizontal scrolling on a small screen is a good way to lose the
 * reader.
 */
type Frame = {
    key: string;
...
const frames = computed<Frame[]>(
    () => group<Frame[]>('occupation.frames') ?? [],
);

/**
 * Two frames carry an upright object rather than a scene: the bronze tree at
 * the Pawiak prison museum and the milk churn that held the Ringelblum
 * archive. Both lose their subject when they are cropped to fill a widescreen
 * frame, so both get their own plate, with the same picture behind them dimmed
 * to a texture.
 */
const exhibits = new Set(['daily', 'revolt']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the two occupation systems carries a wartime administrative map
 * of the General Government, which opens full screen like any other exhibit,
 * because nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'zones';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * All three panels here are upright - two tall photographs and a map printed
 * tall - so they sit in the narrow panel, where they are not surrounded by
 * empty space.
 */
...
<template>
    <Head :title="t('occupation.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('occupation.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/occupation-hero-sm.avif  960w,
                        /images/occupation-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/occupation-hero-sm.jpg"
                    srcset="
                        /images/occupation-hero-sm.jpg  960w,
                        /images/occupation-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/occupation-${frame.photo}-sm.avif 960w, /images/occupation-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/occupation-${frame.photo}-sm.jpg`"
                                :srcset="`/images/occupation-${frame.photo}-sm.jpg 960w, /images/occupation-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/occupation-map-sm.jpg"
                                        srcset="/images/occupation-map-sm.jpg 960w, /images/occupation-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/occupation-map-lg.jpg"
                                        with-caption
                                        :alt="t('occupation.map.alt')"
                                        :caption="t('occupation.map.caption')"
                                        :credit="t('occupation.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/occupation-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/occupation-${frame.photo}-sm.jpg 960w, /images/occupation-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/occupation-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('occupation.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('occupation.sources.body') }}
                </p>
