...
const next = nextAfter('/history/lech-and-the-white-eagle');

/**
 * Where the founding legend comes from and where the eagle comes from,
...
const frames = computed<Frame[]>(() => group<Frame[]>('eagle.frames') ?? []);
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['chronicle', 'legend', 'seal']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/** A frame has a side panel when it holds an exhibit or the emblem. */
const hasPanel = (key: string): boolean => isExhibit(key) || key === 'crown';

/** Backdrops taller than their frame are anchored on the subject. */
const focus: Record<string, string> = {
    hill: 'object-[center_62%]',
};
...
<template>
    <Head :title="t('eagle.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('eagle.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/eagle-hill-sm.avif  760w,
                        /images/eagle-hill-lg.avif 1200w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/eagle-hill-sm.jpg"
                    srcset="
                        /images/eagle-hill-sm.jpg  760w,
                        /images/eagle-hill-lg.jpg 1200w
                    "
                    sizes="100vw"
                    alt=""
                    class="absolute inset-0 h-full w-full object-cover object-[center_62%] opacity-30"
                    loading="eager"
                    decoding="async"
                />
            </picture>
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/eagle-${frame.photo}-sm.avif 760w, /images/eagle-${frame.photo}-lg.avif 1200w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/eagle-${frame.photo}-sm.jpg`"
                                :srcset="`/images/eagle-${frame.photo}-sm.jpg 760w, /images/eagle-${frame.photo}-lg.jpg 1200w`"
                                sizes="100vw"
                                :alt="frame.caption"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <PolishEmblem
                                        v-if="frame.key === 'crown'"
                                        class="mx-auto w-40 sm:w-48"
                                        :label="t('eagle.emblem_alt')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/eagle-${frame.photo}-sm.jpg`"
                                        ...
                                        :full="`/images/eagle-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('eagle.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('eagle.sources.body') }}
                </p>
