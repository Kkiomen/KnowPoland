...
const next = nextAfter('/history/june-1989');

/**
 * How communist Poland ended: a bankrupt state that lost its own referendum in
 * 1987, the secret meetings at Magdalenka, the Round Table of February to April
 * 1989, the eight-week campaign, the partly free election of 4 June, the
 * collapse of the national list, Jaruzelski made president by one vote, the
 * first government in the bloc that the party did not run, and the name and the
 * crowned eagle coming back on 31 December 1989. Told as a filmstrip: scrolling
 * down drives the frames sideways. On a phone the strip becomes an ordinary
 * vertical stack, because horizontal scrolling on a small screen is a good way
 * to lose the reader.
 */
type Frame = {
    key: string;
...
const frames = computed<Frame[]>(() => group<Frame[]>('june1989.frames') ?? []);

/**
 * Almost everything here is a flat sheet that has to be read rather than
 * glanced at: a ration card, an election notice torn at the corners, a
 * polling agent's pass, the letter that made Mazowiecki prime minister.
 * Cropped to fill a widescreen frame all of them lose the detail they are here
 * for, so each is shown whole, on its own plate, with the same picture behind
 * it dimmed to a texture. Only the last frame, a wide view of the museum that
 * now holds the table, works as a backdrop.
 */
...
/**
 * Landscape pictures fill the wide panel. The upright ones - the general's
 * portrait, the letter of appointment and the photograph from Krzyzowa - sit in
 * the narrow panel, where they are not surrounded by empty space.
 */
const upright = new Set(['talks', 'handover', 'government']);
...
<template>
    <Head :title="t('june1989.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('june1989.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/june1989-hero-sm.avif  960w,
                        /images/june1989-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/june1989-hero-sm.jpg"
                    srcset="
                        /images/june1989-hero-sm.jpg  960w,
                        /images/june1989-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/june1989-${frame.photo}-sm.avif 960w, /images/june1989-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/june1989-${frame.photo}-sm.jpg`"
                                :srcset="`/images/june1989-${frame.photo}-sm.jpg 960w, /images/june1989-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/june1989-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('june1989.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('june1989.sources.body') }}
                </p>
