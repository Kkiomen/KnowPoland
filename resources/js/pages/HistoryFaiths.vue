...
import FaithsMap from '@/components/FaithsMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/one-state-many-faiths');

/**
 * The Union of Lublin, the Warsaw Confederation and the faiths of the
 * Commonwealth, told as a filmstrip: scrolling down drives the frames
...
const frames = computed<Frame[]>(() => group<Frame[]>('faiths.frames') ?? []);
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['confederation', 'henry', 'brethren']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the Sejm at Lublin carries the map of the new Commonwealth
 * instead, and its backdrop, Matejko's painting of the union, is dimmed the
 * same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'union';

/** The frame that opens the page carries a real map of the Commonwealth. */
const hasRealMap = (key: string): boolean => key === 'lublin';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean =>
    isExhibit(key) || hasMap(key) || hasRealMap(key);

/** Backdrops taller than their frame are anchored on the subject. */
const focus: Record<string, string> = {
    brest: 'object-[65%_center]',
    neighbours: 'object-[65%_center]',
};
...
<template>
    <Head :title="t('faiths.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('faiths.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/faiths-hero-sm.avif  960w,
                        /images/faiths-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/faiths-hero-sm.jpg"
                    srcset="
                        /images/faiths-hero-sm.jpg  960w,
                        /images/faiths-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/faiths-${frame.photo}-sm.avif 960w, /images/faiths-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/faiths-${frame.photo}-sm.jpg`"
                                :srcset="`/images/faiths-${frame.photo}-sm.jpg 960w, /images/faiths-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                        ...
                        >
                            <div
                                ...
                                :class="
                                    hasMap(frame.key) || hasRealMap(frame.key)
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        v-if="hasRealMap(frame.key)"
                                        src="/images/faiths-commonwealth-sm.jpg"
                                        srcset="/images/faiths-commonwealth-sm.jpg 960w, /images/faiths-commonwealth-lg.jpg 1600w"
                                        sizes="(min-width: 1024px) 34rem, 90vw"
                                        full="/images/faiths-commonwealth-lg.jpg"
                                        :alt="t('faiths.commonwealth.alt')"
                                        :caption="
                                            t('faiths.commonwealth.caption')
                                        "
                                        :credit="
                                            t('faiths.commonwealth.credit')
                                        "
                                        with-caption
                                    />
                                    <FaithsMap
                                    ...
                                    <ExhibitImage
                                        ...
                                        :full="`/images/faiths-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('faiths.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('faiths.sources.body') }}
                </p>
