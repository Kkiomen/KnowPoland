...
const next = nextAfter('/history/government-in-exile');

/**
 * The Polish government in exile, 1939 to 1990: the deep version of the fifth
 * topic in the invasion chapter of the Second World War overview. The
 * internment in Romania and the article of the 1935 constitution that moved the
 * presidency on, Raczkiewicz and Sikorski in Paris and Angers, the fall of
 * France and the move to London, what the government did from there, the
 * agreement with Moscow of 1941 and the army that left the Soviet Union, the
 * break over Katyn in 1943, Sikorski's death at Gibraltar, Tehran and Yalta
 * decided without a Pole in the room, the loss of recognition in July 1945 and
 * the insignia handed back in Warsaw in 1990. The Underground State, Pilecki,
 * Karski, Enigma, the Polish forces abroad and the redrawn borders have their
 * own pages and are only bridged to here.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('exile.frames') ?? []);

/**
 * Two frames carry something that has to be read rather than glanced at: the
 * travel document the last exile president used in London, and the doorway of
 * the museum that holds the archive. Each gets its own plate in the narrow
 * panel, with the same picture behind it dimmed to a texture.
 */
const exhibits = new Set(['end', 'visiting']);
...
/**
 * Everything else runs wide, because the rest of the pictures here are
 * landscape scenes. This page publishes no map: it is about an institution
 * rather than a territory, and the maps of these years are on the September
 * campaign and the pact.
 */
...
<template>
    <Head :title="t('exile.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('exile.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/exile-hero-sm.avif  960w,
                        /images/exile-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/exile-hero-sm.jpg"
                    srcset="
                        /images/exile-hero-sm.jpg  960w,
                        /images/exile-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/exile-${frame.photo}-sm.avif 960w, /images/exile-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/exile-${frame.photo}-sm.jpg`"
                                :srcset="`/images/exile-${frame.photo}-sm.jpg 960w, /images/exile-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        :full="`/images/exile-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('exile.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('exile.sources.body') }}
                </p>
