...
const next = nextAfter('/history/uprisings-1830-1863');

/**
 * The uprisings of 1830 and 1863, from Napoleon's Duchy of Warsaw and the
 * kingdom the tsar was given in 1815 to the November Night, the war of 1831,
 * the emigration, the Citadel, the mourning of 1861, the partisan war of
 * 1863 and what followed it, told as a filmstrip: scrolling down drives the
...
/**
 * The frame on the Congress of Vienna carries the map of 1815, and its
 * backdrop, Schlicht's map of 1831, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'kingdom';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * The map and the four paintings and prints of crowds are wide, so they get
 * the wide panel; Bacciarelli's canvas and the two photographs stand upright
 * and fit the narrow one.
 */
const wide = new Set(['night', 'war', 'exile', 'mourning']);

const isWide = (key: string): boolean => hasMap(key) || wide.has(key);

/** The one photographic backdrop, the Execution Gate, is anchored on its arches. */
const focus: Record<string, string> = {
    citadel: 'object-[50%_center]',
};
...
<template>
    <Head :title="t('uprisings.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('uprisings.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/uprisings-hero-sm.avif  960w,
                        /images/uprisings-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/uprisings-hero-sm.jpg"
                    srcset="
                        /images/uprisings-hero-sm.jpg  960w,
                        /images/uprisings-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/uprisings-${frame.photo}-sm.avif 960w, /images/uprisings-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/uprisings-${frame.photo}-sm.jpg`"
                                :srcset="`/images/uprisings-${frame.photo}-sm.jpg 960w, /images/uprisings-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/uprisings-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('uprisings.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('uprisings.sources.body') }}
                </p>
