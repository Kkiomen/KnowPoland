...
const next = nextAfter('/history/westerplatte');

/**
 * Westerplatte, 1 to 7 September 1939: the deep version of the second topic in
 * the invasion chapter of the Second World War overview. What the depot on the
 * spit actually was, the training ship that moored beside it in August, the
 * first salvo at 4.45, the seven days of attacks, the argument between the two
 * officers that historians still cannot settle, the surrender, the casualties
 * on both sides, what became of the defenders, and what stands on the
 * peninsula now. The Free City itself has its own article and is not retold
 * here, and neither is the rest of the September campaign.
 *
...
const frames = computed<Frame[]>(
    () => group<Frame[]>('westerplatte.frames') ?? [],
);

/**
 * Three frames carry a tall picture rather than a wide scene - the smoke over
 * the burning port workshops, the surrender salute and Sucharski's uniform in
 * its case. A portrait photograph loses its subject when it is cropped to fill
 * a widescreen frame, so each of these gets its own plate in the narrow panel,
 * with the same picture behind it dimmed to a texture.
 */
const exhibits = new Set(['dawn', 'surrender', 'today']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on what the depot was carries a real published sea chart of the
 * harbour mouth, which opens full screen like any other exhibit, because
 * nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'depot';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * Everything that is not a tall plate or the chart runs wide, because the rest
 * of the pictures here are landscape scenes.
 */
...
<template>
    <Head :title="t('westerplatte.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('westerplatte.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/westerplatte-hero-sm.avif  960w,
                        /images/westerplatte-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/westerplatte-hero-sm.jpg"
                    srcset="
                        /images/westerplatte-hero-sm.jpg  960w,
                        /images/westerplatte-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/westerplatte-${frame.photo}-sm.avif 960w, /images/westerplatte-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/westerplatte-${frame.photo}-sm.jpg`"
                                :srcset="`/images/westerplatte-${frame.photo}-sm.jpg 960w, /images/westerplatte-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/westerplatte-map-sm.jpg"
                                        srcset="/images/westerplatte-map-sm.jpg 960w, /images/westerplatte-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/westerplatte-map-lg.jpg"
                                        with-caption
                                        :alt="t('westerplatte.map.alt')"
                                        :caption="t('westerplatte.map.caption')"
                                        :credit="t('westerplatte.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/westerplatte-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/westerplatte-${frame.photo}-sm.jpg 960w, /images/westerplatte-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/westerplatte-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('westerplatte.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('westerplatte.sources.body') }}
                </p>

                <div
                    class="flex flex-wrap items-center justify-between gap-x-8 gap-y-4"
                >
                    <!-- back to the chapter this article hangs off, not to the era index -->
                    <Link
