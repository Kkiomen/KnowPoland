...
import RepublicMap from '@/components/RepublicMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/second-republic-1918');

/**
 * How Poland came back in 1918 and what the Second Republic built before
 * 1939: the three armies of the First World War, the two roads to
 * independence, 11 November, the wars for Lviv and for the western border,
 * one currency out of four, the minorities, Pilsudski's coup and Gdynia,
...
const frames = computed<Frame[]>(() => group<Frame[]>('republic.frames') ?? []);

/**
 * Every picture here is a document of its time - photographs from 1914 to
 * 1934, a letter of the Regency Council, a banknote - and cropping one to fill
 * a widescreen frame destroys the very thing the reader came to see. So each
 * is shown whole, on its own plate, with the same picture behind it dimmed to
 * a texture.
 */
...
/**
 * The frame on the western borders carries the map of Poland in 1922 with
 * the old partitions inside it, and its backdrop, the bilingual plebiscite
 * poster, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'silesia';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * The map and the landscape photographs get the wide panel; the Regency
 * Council's upright letter fits the narrow one.
 */
const isWide = (key: string): boolean => key !== 'november';
...
<template>
    <Head :title="t('republic.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('republic.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/republic-hero-sm.avif  960w,
                        /images/republic-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/republic-hero-sm.jpg"
                    srcset="
                        /images/republic-hero-sm.jpg  960w,
                        /images/republic-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/republic-${frame.photo}-sm.avif 960w, /images/republic-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/republic-${frame.photo}-sm.jpg`"
                                :srcset="`/images/republic-${frame.photo}-sm.jpg 960w, /images/republic-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/republic-atlas1920-sm.jpg"
                                        srcset="/images/republic-atlas1920-sm.jpg 960w, /images/republic-atlas1920-lg.jpg 1600w"
                                        sizes="(min-width: 1024px) 34rem, 90vw"
                                        full="/images/republic-atlas1920-lg.jpg"
                                        :alt="t('republic.periodmap.alt')"
                                        :caption="
                                            t('republic.periodmap.caption')
                                        "
                                        :credit="t('republic.periodmap.credit')"
                                        with-caption
                                    />
                                    <ExhibitImage
                                        v-else
                                        ...
                                        "
                                        :full="`/images/republic-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('republic.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('republic.sources.body') }}
                </p>
