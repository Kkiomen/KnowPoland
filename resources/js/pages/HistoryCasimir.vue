...
import CasimirMap from '@/components/CasimirMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/casimir-the-great');

/**
 * Casimir the Great, from his father's reunited kingdom to the tomb opened
 * by accident in 1869, told as a filmstrip: scrolling down drives the frames sideways. On a phone the strip becomes an ordinary vertical stack, because
...
const frames = computed<Frame[]>(() => group<Frame[]>('casimir.frames') ?? []);
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['father', 'peace', 'law', 'end']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the turn east carries the map instead, and its backdrop, the
 * Lviv charter, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'east';
...
const focus: Record<string, string> = {
    brick: 'object-[70%_center]',
    today: 'object-[center_60%]',
};
...
<template>
    <Head :title="t('casimir.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('casimir.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/casimir-hero-sm.avif  960w,
                        /images/casimir-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/casimir-hero-sm.jpg"
                    srcset="
                        /images/casimir-hero-sm.jpg  960w,
                        /images/casimir-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/casimir-${frame.photo}-sm.avif 960w, /images/casimir-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/casimir-${frame.photo}-sm.jpg`"
                                :srcset="`/images/casimir-${frame.photo}-sm.jpg 960w, /images/casimir-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        :full="`/images/casimir-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('casimir.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('casimir.sources.body') }}
                </p>
