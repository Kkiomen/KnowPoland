...
const next = nextAfter('/history/molotov-ribbentrop-pact');

/**
 * The German-Soviet pact of 23 August 1939: the deep version of the fourth
 * topic in the invasion chapter of the Second World War overview. The Moscow
 * military talks that failed over the transit question, the night of the
 * signing, what the secret protocol actually said, how the line was redrawn on
 * 28 September, the Baltic states, Bessarabia and Finland, the trade that ran
 * until June 1941, fifty years of Soviet denial and the admission of 1989, how
 * the pact is remembered now, and where to read the documents. The September
 * campaign, the Free City of Danzig and Westerplatte have their own pages and
 * are not retold here.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('pact.frames') ?? []);

/**
 * Two frames carry a tall picture rather than a wide scene - the studio
 * portrait of Voroshilov and the printed journal page that finally carried the
 * Soviet originals. A portrait or a page of type loses its subject when it is
 * cropped to fill a widescreen frame, so each of these gets its own plate in
 * the narrow panel, with the same picture behind it dimmed to a texture.
 */
const exhibits = new Set(['talks', 'denial']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the September border carries the map the Soviet daily Izvestia
 * printed in 1939, which opens full screen like any other exhibit, because
 * nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'poland';
...
<template>
    <Head :title="t('pact.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('pact.meta.description')"
        />
    </Head>
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/pact-hero-sm.avif  960w,
                        /images/pact-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/pact-hero-sm.jpg"
                    srcset="
                        /images/pact-hero-sm.jpg  960w,
                        /images/pact-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/pact-${frame.photo}-sm.avif 960w, /images/pact-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/pact-${frame.photo}-sm.jpg`"
                                :srcset="`/images/pact-${frame.photo}-sm.jpg 960w, /images/pact-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/pact-map-sm.jpg"
                                        srcset="/images/pact-map-sm.jpg 960w, /images/pact-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/pact-map-lg.jpg"
                                        with-caption
                                        :alt="t('pact.map.alt')"
                                        :caption="t('pact.map.caption')"
                                        :credit="t('pact.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/pact-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/pact-${frame.photo}-sm.jpg 960w, /images/pact-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/pact-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
