...
const next = nextAfter('/history/serfdom');

/**
 * Serfdom in the Commonwealth, from the lord's home farm to the decrees
 * that ended it in each partition, told as a filmstrip: scrolling down drives the frames
...
const frames = computed<Frame[]>(() => group<Frame[]>('serfdom.frames') ?? []);
 ...
 * print, a manuscript page, an archive register - is an exhibit, and cropping it to
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['dues', 'court', 'polaniec', 'duchy', 'decree']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * Backdrops taller than their frame are anchored on the subject. On a phone
 * the inscribed cross would sit under the text, so it is pushed out of view.
 */
const focus: Record<string, string> = {
    galicia: 'object-[90%_center] lg:object-[55%_center]',
    today: 'object-[60%_center]',
};
...
<template>
    <Head :title="t('serfdom.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('serfdom.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/serfdom-hero-sm.avif  960w,
                        /images/serfdom-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/serfdom-hero-sm.jpg"
                    srcset="
                        /images/serfdom-hero-sm.jpg  960w,
                        /images/serfdom-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/serfdom-${frame.photo}-sm.avif 960w, /images/serfdom-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/serfdom-${frame.photo}-sm.jpg`"
                                :srcset="`/images/serfdom-${frame.photo}-sm.jpg 960w, /images/serfdom-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        :full="`/images/serfdom-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('serfdom.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('serfdom.sources.body') }}
                </p>
