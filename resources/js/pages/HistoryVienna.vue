...
import ViennaMap from '@/components/ViennaMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/vienna-1683');

/**
 * The road to Vienna in 1683, from the Cossack rising and the Deluge to the
 * relief of the siege and what it brought, told as a filmstrip: scrolling down drives the frames
...
const frames = computed<Frame[]>(() => group<Frame[]>('vienna.frames') ?? []);
 ...
 * portrait, a siege plan, a suit of armour - is an exhibit, and cropping it to
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['deluge', 'vizier', 'siege', 'charge']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the relief march carries the map of the route instead, and
 * its backdrop, the hills above Vienna, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'march';

/**
 * The siege plan is a wide sheet, so like the map it gets the wide panel;
 * squeezed into the narrow one it shrinks to a postage stamp.
 */
const isWide = (key: string): boolean => hasMap(key) || key === 'siege';
...
const focus: Record<string, string> = {
    khotyn: 'object-[60%_center]',
    kamianets: 'object-[60%_center]',
};
...
<template>
    <Head :title="t('vienna.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('vienna.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/vienna-hero-sm.avif  960w,
                        /images/vienna-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/vienna-hero-sm.jpg"
                    srcset="
                        /images/vienna-hero-sm.jpg  960w,
                        /images/vienna-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/vienna-${frame.photo}-sm.avif 960w, /images/vienna-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/vienna-${frame.photo}-sm.jpg`"
                                :srcset="`/images/vienna-${frame.photo}-sm.jpg 960w, /images/vienna-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/vienna-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('vienna.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('vienna.sources.body') }}
                </p>
