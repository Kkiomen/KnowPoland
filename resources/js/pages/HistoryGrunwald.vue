...
import GrunwaldMap from '@/components/GrunwaldMap.vue';
import { nextAfter } from '@/history-articles';
import { group, t } from '@/i18n';

const next = nextAfter('/history/grunwald-1410');

/**
 * Grunwald, 1410, from the knights a Polish duke invited in to the field
 * where the battle is fought again every July, told as a filmstrip: scrolling down drives the frames sideways. On a phone the strip becomes an ordinary vertical stack, because
...
const frames = computed<Frame[]>(() => group<Frame[]>('grunwald.frames') ?? []);
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['krewo', 'vytautas', 'battle', 'peace']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the march to the battle carries the map instead, and its
 * backdrop, the Vistula at Czerwinsk, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'march';

/** The frame on the Order's state carries a period map of Prussia. */
const hasPrussia = (key: string): boolean => key === 'order';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean =>
    isExhibit(key) || hasMap(key) || hasPrussia(key);

/** Backdrops taller than their frame are anchored on the subject. */
const focus: Record<string, string> = {
    order: 'object-[70%_center]',
    thirteen: 'object-[65%_center]',
};
...
<template>
    <Head :title="t('grunwald.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('grunwald.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/grunwald-hero-sm.avif  960w,
                        /images/grunwald-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/grunwald-hero-sm.jpg"
                    srcset="
                        /images/grunwald-hero-sm.jpg  960w,
                        /images/grunwald-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/grunwald-${frame.photo}-sm.avif 960w, /images/grunwald-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/grunwald-${frame.photo}-sm.jpg`"
                                :srcset="`/images/grunwald-${frame.photo}-sm.jpg 960w, /images/grunwald-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                        ...
                        >
                            <div
                                ...
                                :class="
                                    hasMap(frame.key) || hasPrussia(frame.key)
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        v-if="hasPrussia(frame.key)"
                                        src="/images/grunwald-prussia-sm.jpg"
                                        srcset="/images/grunwald-prussia-sm.jpg 960w, /images/grunwald-prussia-lg.jpg 1600w"
                                        sizes="(min-width: 1024px) 34rem, 90vw"
                                        full="/images/grunwald-prussia-lg.jpg"
                                        :alt="t('grunwald.periodmap.alt')"
                                        :caption="
                                            t('grunwald.periodmap.caption')
                                        "
                                        :credit="t('grunwald.periodmap.credit')"
                                        with-caption
                                    />
                                    <GrunwaldMap
                                        v-else-if="hasMap(frame.key)"
                                    ...
                                    <ExhibitImage
                                        ...
                                        :full="`/images/grunwald-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('grunwald.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('grunwald.sources.body') }}
                </p>
