...
const next = nextAfter('/history/rebuilding-warsaw');

/**
 * How Warsaw was put back after 1945: the scale of the ruin, the proposal to
 * move the capital to Lodz, the Warsaw Reconstruction Office, the conservation
 * rule Jan Zachwatowicz broke, the paintings and measured drawings the old town
 * was copied from, the bricks that came from other cities, what is genuinely
 * old and what was invented, the castle that waited forty years, the socialist
 * city built alongside, and what UNESCO made of the result in 1980. Told as a
 * filmstrip: scrolling down drives the frames sideways. On a phone the strip
 * becomes an ordinary vertical stack, because horizontal scrolling on a small
 * screen is a good way to lose the reader.
 */
type Frame = {
    key: string;
    photo: string;
...
const frames = computed<Frame[]>(
    () => group<Frame[]>('rebuilding.frames') ?? [],
);

/**
 * Most of the evidence here is a flat sheet: an archival photograph, a
 * drawing board, a painted veduta. Cropped to fill a widescreen frame all of
 * them lose the detail they are here for, so each is shown whole, on its own
...
/**
 * The frame on the ruined city carries the period plan of Warsaw, which opens
 * full screen like any other exhibit, because nobody can read a map at panel
 * size.
 */
const hasMap = (key: string): boolean => key === 'rubble';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * Landscape pictures fill the wide panel. The upright ones - the portrait of
 * Jan Zachwatowicz and the tall sheet of the 1949 destruction map - sit in the
 * narrow panel, where they are not surrounded by empty space.
 */
const isWide = (key: string): boolean => key !== 'doctrine' && key !== 'rubble';
...
<template>
    <Head :title="t('rebuilding.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('rebuilding.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/rebuilding-hero-sm.avif  960w,
                        /images/rebuilding-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/rebuilding-hero-sm.jpg"
                    srcset="
                        /images/rebuilding-hero-sm.jpg  960w,
                        /images/rebuilding-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/rebuilding-${frame.photo}-sm.avif 960w, /images/rebuilding-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/rebuilding-${frame.photo}-sm.jpg`"
                                :srcset="`/images/rebuilding-${frame.photo}-sm.jpg 960w, /images/rebuilding-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/rebuilding-map-sm.jpg"
                                        srcset="/images/rebuilding-map-sm.jpg 960w, /images/rebuilding-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/rebuilding-map-lg.jpg"
                                        :alt="t('rebuilding.map.alt')"
                                        :caption="t('rebuilding.map.caption')"
                                        :credit="t('rebuilding.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/rebuilding-${frame.photo}-sm.jpg`"
                                        ...
                                        "
                                        :full="`/images/rebuilding-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('rebuilding.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('rebuilding.sources.body') }}
                </p>
