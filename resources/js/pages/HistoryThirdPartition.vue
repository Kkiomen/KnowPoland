...
const next = nextAfter('/history/third-partition-1795');

/**
 * The Third Partition of 1795, from the capitulation of Warsaw to the
 * treaties, the abdication at Grodno, the convention that meant to abolish
 * the name, the legions in Italy and the 123 years that followed, told as a
 * filmstrip: scrolling down drives the frames sideways. On a phone the strip becomes an
...
/**
 * The frame on the shares carries the partition map with all three
 * partitions on it, and its backdrop, Cruttwell's map of 1799, is dimmed the
 * same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'shares';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * The map, Cary's map of 1799 and the Milan engraving are wide, so they get
 * the wide panel; the four portraits stand upright and fit the narrow one.
 */
const isWide = (key: string): boolean =>
    hasMap(key) || key === 'name' || key === 'legions';

/**
 * The two photographic backdrops are anchored on their subjects: the
 * grave plate and monogram in St Petersburg, the palace across the pond.
 */
const focus: Record<string, string> = {
    petersburg: 'object-[35%_center]',
    today: 'object-[45%_center]',
};
...
<template>
    <Head :title="t('partition3.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('partition3.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/partition3-hero-sm.avif  960w,
                        /images/partition3-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/partition3-hero-sm.jpg"
                    srcset="
                        /images/partition3-hero-sm.jpg  960w,
                        /images/partition3-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/partition3-${frame.photo}-sm.avif 960w, /images/partition3-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/partition3-${frame.photo}-sm.jpg`"
                                :srcset="`/images/partition3-${frame.photo}-sm.jpg 960w, /images/partition3-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                <div class="max-w-[46ch]">
                                    ...
                                    <ExhibitImage
                                        ...
                                        :srcset="`/images/partition3-${frame.photo}-sm.jpg 960w, /images/partition3-${frame.photo}-lg.jpg 1280w`"
                                        sizes="90vw"
                                        :full="`/images/partition3-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
                                ...
                                >
                                    <ExhibitImage
                                        v-if="hasMap(frame.key)"
                                        src="/images/partition3-faden-sm.jpg"
                                        srcset="/images/partition3-faden-sm.jpg 960w, /images/partition3-faden-lg.jpg 1600w"
                                        sizes="(min-width: 1024px) 34rem, 90vw"
                                        full="/images/partition3-faden-lg.jpg"
                                        :alt="t('partition3.periodmap.alt')"
                                        :caption="
                                            t('partition3.periodmap.caption')
                                        "
                                        :credit="
                                            t('partition3.periodmap.credit')
                                        "
                                        with-caption
                                    />
                                    <ExhibitImage
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('partition3.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('partition3.sources.body') }}
                </p>
