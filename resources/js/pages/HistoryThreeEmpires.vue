...
const next = nextAfter('/history/three-empires-three-cities');

/**
 * What the three partitions left in stone, iron and habit, read city by
 * city: the corner where the three empires met, Prussian Poznan with its
 * forts and the Kaiser's castle, Austrian Krakow with the Planty, the
 * barracks on Wawel and Galician poverty, Russian Warsaw with its lost
 * cathedral and its two railway gauges, and the old border that still shows
 * on election maps, told as a filmstrip: scrolling down drives the frames
...
const frames = computed<Frame[]>(() => group<Frame[]>('empires.frames') ?? []);

/**
 * Every picture here is a document of its time - postcards, a painting, a
 * studio portrait, an election map - and cropping one to fill a widescreen
 * frame destroys the very thing the reader came to see. So each is shown
 * whole, on its own plate, with the same picture behind it dimmed to a
 * texture.
 */
...
/**
 * The opening frame carries the map of the three partitions around 1900,
 * and its backdrop, a postcard of the Three Emperors' Corner, is dimmed the
 * same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'corner';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * The map and the six postcards, photographs and paintings of places are
 * wide, so they get the wide panel; Szczepanowski's portrait and the square
 * election map fit the narrow one.
 */
const wide = new Set([
    'fortress',
    'castle',
...
const isWide = (key: string): boolean => hasMap(key) || wide.has(key);
...
<template>
    <Head :title="t('empires.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('empires.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/empires-hero-sm.avif  960w,
                        /images/empires-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/empires-hero-sm.jpg"
                    srcset="
                        /images/empires-hero-sm.jpg  960w,
                        /images/empires-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/empires-${frame.photo}-sm.avif 960w, /images/empires-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/empires-${frame.photo}-sm.jpg`"
                                :srcset="`/images/empires-${frame.photo}-sm.jpg 960w, /images/empires-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/empires-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('empires.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('empires.sources.body') }}
                </p>
