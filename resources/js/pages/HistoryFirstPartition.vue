...
const next = nextAfter('/history/first-partition-1772');

/**
 * The First Partition of 1772, from Russia's Turkish war to the Sejm that
 * ratified it and the Permanent Council it left behind, told as a filmstrip: scrolling down drives the frames
 ...
 * dimmed to a texture.
 */
const exhibits = new Set(['henry', 'treaty', 'vistula', 'council']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on the three shares carries the partition map instead, and its
 * backdrop, a sheet of Rizzi Zannoni's map of 1772, is dimmed the same way
 * an exhibit's is.
 */
...
/**
 * The map and Deisch's panorama of Gdansk are wide, so they get the wide
 * panel; squeezed into the narrow one they shrink to a postage stamp.
 */
const isWide = (key: string): boolean => hasMap(key) || key === 'vistula';

/**
 * Backdrops wider than their frame are anchored on the subject: Rejtan on
 * the floor at the right of Matejko's canvas, the castle on its hill.
 */
const focus: Record<string, string> = {
    sejm: 'object-[80%_center]',
    zips: 'object-[70%_center]',
};
...
<template>
    <Head :title="t('partition1.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('partition1.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/partition1-hero-sm.avif  960w,
                        /images/partition1-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/partition1-hero-sm.jpg"
                    srcset="
                        /images/partition1-hero-sm.jpg  960w,
                        /images/partition1-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/partition1-${frame.photo}-sm.avif 960w, /images/partition1-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/partition1-${frame.photo}-sm.jpg`"
                                :srcset="`/images/partition1-${frame.photo}-sm.jpg 960w, /images/partition1-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                <div class="max-w-[46ch]">
                                    ...
                                    <ExhibitImage
                                        ...
                                        :srcset="`/images/partition1-${frame.photo}-sm.jpg 960w, /images/partition1-${frame.photo}-lg.jpg 1280w`"
                                        sizes="90vw"
                                        :full="`/images/partition1-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('partition1.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('partition1.sources.body') }}
                </p>
