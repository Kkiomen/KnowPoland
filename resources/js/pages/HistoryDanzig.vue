...
const next = nextAfter('/history/free-city-of-danzig');

/**
 * The Free City of Danzig, 1920-1939: the deep version of the first topic in
 * the invasion chapter of the Second World War overview. What the Free City
 * legally was, why the peace conference invented it, what rights Poland held
 * inside it, the running quarrels, why Poland built Gdynia instead, the Nazi
 * takeover from 1933, the crisis of 1939, the end of the city in September
 * 1939 and 1945, and what a visitor finds in Gdansk today. It deliberately
 * does not retell the September campaign, which has its own article.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('danzig.frames') ?? []);

/**
 * Two frames carry a flat document rather than a scene: the Free City's own
 * state loan bond, which says the whole constitutional arrangement on one
 * sheet, and a Danzig passport with the British visa that took children out in
 * 1939. A scan loses everything when it is cropped to fill a widescreen frame,
 * so each gets its own plate, with the same picture behind it dimmed to a
 * texture.
 */
const exhibits = new Set(['what', 'crisis']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on why the Free City was invented carries a real published map of
 * it and of the corridor, which opens full screen like any other exhibit,
 * because nobody can read a map at panel size.
 */
const hasMap = (key: string): boolean => key === 'why';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * Both panels here are sheets rather than scenes - a scan of money and stamps,
 * and a printed map - so both sit in the narrow panel, where they are not
...
<template>
    <Head :title="t('danzig.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('danzig.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/danzig-hero-sm.avif  960w,
                        /images/danzig-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/danzig-hero-sm.jpg"
                    srcset="
                        /images/danzig-hero-sm.jpg  960w,
                        /images/danzig-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/danzig-${frame.photo}-sm.avif 960w, /images/danzig-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/danzig-${frame.photo}-sm.jpg`"
                                :srcset="`/images/danzig-${frame.photo}-sm.jpg 960w, /images/danzig-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/danzig-map-sm.jpg"
                                        srcset="/images/danzig-map-sm.jpg 960w, /images/danzig-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/danzig-map-lg.jpg"
                                        with-caption
                                        :alt="t('danzig.map.alt')"
                                        :caption="t('danzig.map.caption')"
                                        :credit="t('danzig.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/danzig-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/danzig-${frame.photo}-sm.jpg 960w, /images/danzig-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/danzig-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('danzig.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('danzig.sources.body') }}
                </p>
