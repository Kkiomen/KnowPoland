...
const next = nextAfter('/history/kosciuszko-1794');

/**
 * Tadeusz Kosciuszko and the rising of 1794, from the engineer of Saratoga
 * and West Point to the oath in Krakow, the scythemen, the gallows, the
 * siege, Praga and the mound Krakow raised for him, told as a filmstrip:
...
/**
 * The frame on the summer of 1794 carries the partition map with the
 * battles of the rising marked on it, and its backdrop, Plersch's painting
 * of Maciejowice, is dimmed the same way an exhibit's is.
 */
const hasMap = (key: string): boolean => key === 'siege';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/**
 * Every picture on this page is wider than it is tall, and L'Enfant's
 * panorama of West Point most of all, so every panel gets the wide size;
 * squeezed into the narrow one they shrink to a postage stamp.
 */
const isWide = (key: string): boolean => hasPanel(key);

/**
 * The one photographic backdrop is anchored on its subject, the mound in
 * the middle of the fort around it.
 */
const focus: Record<string, string> = {
    memory: 'object-[60%_center]',
};
...
<template>
    <Head :title="t('kosciuszko.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('kosciuszko.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/kosciuszko-hero-sm.avif  960w,
                        /images/kosciuszko-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/kosciuszko-hero-sm.jpg"
                    srcset="
                        /images/kosciuszko-hero-sm.jpg  960w,
                        /images/kosciuszko-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/kosciuszko-${frame.photo}-sm.avif 960w, /images/kosciuszko-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/kosciuszko-${frame.photo}-sm.jpg`"
                                :srcset="`/images/kosciuszko-${frame.photo}-sm.jpg 960w, /images/kosciuszko-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <ExhibitImage
                                        ...
                                        "
                                        :full="`/images/kosciuszko-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('kosciuszko.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('kosciuszko.sources.body') }}
                </p>
