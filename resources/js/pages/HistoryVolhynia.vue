...
const next = nextAfter('/history/volhynia-1943');

/**
 * Volhynia and Eastern Galicia, 1943 to 1945: the massacres of the Polish
 * population by the Ukrainian Insurgent Army, the Polish reprisals, and why the
 * name Bandera still stops a conversation in Poland. Thirteen frames, from how
 * Poles and Ukrainians came to share the same villages, through the interwar
 * Polish state, the OUN, the two occupations, the coordinated attacks of 11
 * July 1943, the self-defence villages and the Ukrainians who saved their
 * neighbours, the Polish reprisals, the disputed casualty counts, where Bandera
 * actually was, the silence under communism and the exhumations now.
 *
 * Every number on the page is attributed, and where Polish and Ukrainian
 * scholarship disagree the copy says so instead of picking a side.
 *
 * The General Government, the Holocaust and the postwar border changes have
 * their own articles, and they are only bridged to here.
 *
...
const frames = computed<Frame[]>(() => group<Frame[]>('volhynia.frames') ?? []);

/**
 * Two frames carry an upright picture rather than a scene: the 1934 funeral of
 * the minister the OUN had shot, and the monument standing in Lviv today. Both
 * are documents rather than backdrops, so each gets its own plate with the same
 * picture behind it dimmed to a texture.
 */
const exhibits = new Set(['oun', 'bandera']);

const isExhibit = (key: string): boolean => exhibits.has(key);

/**
 * The frame on 11 July 1943 carries a real map of the country it happened in,
 * sheet 77 of the Polish army's operational map printed in 1928, which opens
 * full screen like any other exhibit, because nobody can read a map at panel
 * size.
 */
const hasMap = (key: string): boolean => key === 'july';

/** A frame has a side panel when it holds an exhibit or the map. */
const hasPanel = (key: string): boolean => isExhibit(key) || hasMap(key);

/** Both panels here are upright, so they sit in the narrow column. */
...
<template>
    <Head :title="t('volhynia.meta.title')">
        <meta
            head-key="description"
            name="description"
            :content="t('volhynia.meta.description')"
        />
    </Head>
    ...
    <main class="bg-ink">
        ...
        <section class="relative overflow-hidden">
            <picture>
                <source
                    type="image/avif"
                    srcset="
                        /images/volhynia-hero-sm.avif  960w,
                        /images/volhynia-hero-lg.avif 1280w
                    "
                    sizes="100vw"
                />
                <img
                    src="/images/volhynia-hero-sm.jpg"
                    srcset="
                        /images/volhynia-hero-sm.jpg  960w,
                        /images/volhynia-hero-lg.jpg 1280w
                    "
                    sizes="100vw"
                    alt=""
            ...
            <div class="film:sticky film:top-0 film:h-dvh film:overflow-hidden">
                ...
                >
                    ...
                    >
                        <picture>
                            <source
                                type="image/avif"
                                :srcset="`/images/volhynia-${frame.photo}-sm.avif 960w, /images/volhynia-${frame.photo}-lg.avif 1280w`"
                                sizes="100vw"
                            />
                            <img
                                :src="`/images/volhynia-${frame.photo}-sm.jpg`"
                                :srcset="`/images/volhynia-${frame.photo}-sm.jpg 960w, /images/volhynia-${frame.photo}-lg.jpg 1280w`"
                                sizes="100vw"
                                :alt="frame.caption"
                            ...
                            >
                                ...
                                >
                                    <ExhibitImage
                                        ...
                                        src="/images/volhynia-map-sm.jpg"
                                        srcset="/images/volhynia-map-sm.jpg 960w, /images/volhynia-map-lg.jpg 1280w"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        full="/images/volhynia-map-lg.jpg"
                                        with-caption
                                        :alt="t('volhynia.map.alt')"
                                        :caption="t('volhynia.map.caption')"
                                        :credit="t('volhynia.map.credit')"
                                    />
                                    <ExhibitImage
                                        v-else
                                        :src="`/images/volhynia-${frame.photo}-sm.jpg`"
                                        :srcset="`/images/volhynia-${frame.photo}-sm.jpg 960w, /images/volhynia-${frame.photo}-lg.jpg 1280w`"
                                        sizes="(min-width: 1024px) 22rem, 90vw"
                                        :full="`/images/volhynia-${frame.photo}-lg.jpg`"
                                        :alt="frame.caption"
                ...
                >
                    <div class="mx-auto w-full max-w-[1200px]">
                        ...
                        <div
                            class="pointer-events-auto mt-3 flex flex-wrap justify-between gap-x-3 gap-y-2"
                        >
                            <button
                                v-for="(frame, index) in frames"
                                :key="frame.key"
        ...
        <section class="border-hairline border-t">
            <div class="mx-auto w-full max-w-[1200px] px-6 py-14 sm:py-16">
                ...
                >
                    {{ t('volhynia.sources.label') }}
                </p>
                <p class="text-bone-muted mt-3 max-w-[70ch]">
                    {{ t('volhynia.sources.body') }}
                </p>
